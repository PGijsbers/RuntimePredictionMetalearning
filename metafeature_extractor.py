	...
	try:
		...
		for i in np.arange(0.1, 1.01, 0.1):
			...
			if(int(i*len(y)) >= 500):
				log("Subsample.")
				if i != 1:
					split = StratifiedShuffleSplit(y, 1, test_size=(1-i), random_state = 0)
					train_indices, _ = next(split.__iter__())
				else:
				...
				folds = StratifiedShuffleSplit(y_s, n_iter=10, test_size=0.2, random_state = 0)
